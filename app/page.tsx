...
import { 
  ...
  RefreshCcw, ChevronRight, Fingerprint, Map, Activity
} from 'lucide-react';
...
// Utility for class merging (simulating clsx + tailwind-merge)
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const storedSessions = localStorage.getItem('axaxyz_sessions');
    const storedLogs = localStorage.getItem('axaxyz_logs');
    if (storedSessions) setSessions(JSON.parse(storedSessions));
    else setSessions(defaultSessions);
  ...
  const saveSessions = (newSessions: Session[]) => {
    ...
    localStorage.setItem('axaxyz_sessions', JSON.stringify(newSessions));
  };

  const saveLogs = (newLogs: Log[]) => {
    setLogs(newLogs);
    localStorage.setItem('axaxyz_logs', JSON.stringify(newLogs));
  };
  ...
  const addSession = (sessionData: Omit<Session, 'id'>) => {
    saveSessions([...sessions, { ...sessionData, id: Math.random().toString(36).substr(2, 9) }]);
  };
  ...
  return (
    <AppContext.Provider value={{ sessions, logs, addLog, updateSession, addSession, deleteSession }}>
      {children}
    </AppContext.Provider>
  );
};
...
const TimeCheck: React.FC<{ onComplete: (data: { sessionName: string; status: 'Hadir' | 'Terlambat' }) => void }> = ({ onComplete }) => {
  ...
  const activeSession = useMemo(() => {
    ...
    for (const session of sessions) {
      ...
      // If current time is within session window
        ...
        {activeSession ? (
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              ...
              <span className={cn(
                ...
                activeSession.status === 'Hadir' ? "bg-emerald-500/20 text-emerald-400" : "bg-orange-500/20 text-orange-400"
              )}>
                {activeSession.status}
              </span>
            </div>
            <button 
              onClick={() => onComplete({ sessionName: activeSession.session.name, status: activeSession.status as 'Hadir' | 'Terlambat' })}
              className="w-full py-4 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(168,85,247,0.5)] flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-4 text-center">
            ...
            <div className="text-left mt-4">
              <p className="text-sm text-slate-400 mb-2">Jadwal Sesi Berikutnya:</p>
              {sessions.filter(s => s.isActive).map(s => (
                <div key={s.id} className="flex justify-between text-sm py-1 border-b border-white/5 last:border-0">
                  <span>{s.name}</span>
                  <span className="text-cyan-400">{s.startTime} - {s.endTime}</span>
                </div>
              ))}
            </div>
          </div>
...
const LocationCheck: React.FC<{ onComplete: (loc: {lat: number, lng: number}) => void }> = ({ onComplete }) => {
  ...
  // Target coordinates (Campus - Example: Jakarta)
  const TARGET_LAT = -6.200000;
  const TARGET_LNG = 106.816666;
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (dist <= RADIUS_METERS) {
          ...
          setStatus('error');
          setErrorMsg(`Anda berada di luar area kampus.`);
        }
      },
      (error) => {
        setStatus('error');
      ...
      <div className="text-center space-y-2 w-full bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-2xl shadow-xl">
        ...
        {status === 'success' && (
          <div className="text-emerald-400 space-y-2 animate-in fade-in zoom-in">
            ...
            <p className="text-sm text-emerald-500/80">Jarak: {Math.round(distance || 0)} meter dari titik pusat.</p>
          </div>
        )}
        ...
        {status === 'error' && (
          <div className="space-y-4">
            <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg">
              ...
              {distance && <p className="text-xs text-rose-500/70 mt-1">Jarak saat ini: {Math.round(distance)}m (Maks: {RADIUS_METERS}m)</p>}
            </div>
            
            <div className="flex gap-2">
              <button 
                onClick={checkLocation}
                className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors"
              >
                <RefreshCcw className="w-4 h-4" /> Coba Lagi
              </button>
              
              {/* Bypass button for testing in preview environment */}
              <button 
                onClick={() => onComplete({ lat: TARGET_LAT, lng: TARGET_LNG })}
                className="flex-1 py-3 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 border border-cyan-500/50 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors"
              >
...
// Step 3: QR Scanner & Anti-Fraud
const QRScanner: React.FC<{ onComplete: (data: {nim: string, name: string, deviceId: string}) => void }> = ({ onComplete }) => {
  const [nimInput, setNimInput] = useState('');
  const [nameInput, setNameInput] = useState('Mahasiswa Demo');
  const [error, setError] = useState('');
  const [scanMode, setScanMode] = useState(false);

  const handleSimulateScan = () => {
    setError('');
    if (!nimInput || nimInput.length < 5) {
      setError('Format QR Tidak Valid. Masukkan NIM yang benar.');
      return;
    }
    ...
    if (!deviceOwner) {
      // First time use on this device
      localStorage.setItem('axaxyz_device_owner', nimInput);
    } else if (deviceOwner !== nimInput) {
      // Fraud detected!
      setError('⚠️ Fraud Alert: Perangkat ini sudah terdaftar untuk mahasiswa lain (NIM berbeda). Akses ditolak.');
      return;
    }

    onComplete({ nim: nimInput, name: nameInput, deviceId });
  };
    ...
    <div className="flex flex-col items-center justify-center p-8 space-y-6 max-w-md mx-auto animate-in slide-in-from-right duration-500">
      <div className="w-full bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-xl">
        <div className="text-center mb-8">
          ...
          <h3 className="text-2xl font-bold text-white mb-2">Scan ID Card</h3>
          <p className="text-slate-400 text-sm">Pindai QR Code pada Kartu Tanda Mahasiswa (KTM) Anda.</p>
        </div>

        {scanMode ? (
          <div className="aspect-square bg-black rounded-xl border-2 border-dashed border-white/20 flex flex-col items-center justify-center p-4 relative overflow-hidden group">
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20 mix-blend-overlay"></div>
            <div className="absolute w-full h-1 bg-cyan-500/50 blur-sm top-1/2 -translate-y-1/2 animate-scan"></div>
            <QrCode className="w-12 h-12 text-white/30 mb-2" />
            <p className="text-xs text-white/50 text-center">Menunggu Akses Kamera...<br/>(Simulasi diaktifkan untuk preview)</p>
            <button onClick={() => setScanMode(false)} className="absolute bottom-4 text-xs text-cyan-400 underline z-10">Batal / Gunakan Input Manual</button>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Simulasi Data QR (NIM)</label>
              <div className="flex items-center bg-slate-900/50 border border-white/10 rounded-xl overflow-hidden focus-within:border-cyan-500 transition-colors">
                ...
                <input 
                  ...
                  className="w-full bg-transparent py-3 pr-4 text-white outline-none placeholder-slate-600"
                  value={nimInput}
            ...
            <button 
              ...
              className="w-full py-3.5 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-bold rounded-xl transition-all shadow-[0_0_15px_rgba(6,182,212,0.2)]"
            >
              Proses Data QR
            </button>
          </div>
      ...
      {!scanMode && (
         <p className="text-xs text-slate-500 text-center max-w-xs">
           Sistem dilengkapi Device Fingerprinting. Satu perangkat hanya dapat digunakan untuk satu NIM.
         </p>
      )}
    </div>
  );
      ...
      <div className="w-full bg-slate-900 rounded-3xl overflow-hidden border-4 border-white/10 relative shadow-2xl aspect-[3/4] md:aspect-video flex items-center justify-center bg-black">
        {error ? (
          <div className="p-6 text-center space-y-4">
            ...
            {/* Fallback for environments blocking camera */}
            <button onClick={() => {
              const dummyCanvas = document.createElement('canvas');
              dummyCanvas.width = 400; dummyCanvas.height = 300;
              const ctx = dummyCanvas.getContext('2d');
              if(ctx) {
                ctx.fillStyle = '#1e293b'; ctx.fillRect(0, 0, 400, 300);
                ctx.fillStyle = '#06b6d4'; ctx.font = '20px sans-serif'; ctx.fillText('Dummy Image', 130, 150);
              }
              onComplete(dummyCanvas.toDataURL());
        ...
        <div className="w-full">
          {!image ? (
            ...
            <div className="flex gap-4">
              <button onClick={retake} className="flex-1 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-2xl transition-all">
                Ulangi
              </button>
...
const SuccessScreen: React.FC<{ reset: () => void }> = ({ reset }) => (
  <div className="flex flex-col items-center justify-center p-8 space-y-8 text-center animate-in zoom-in duration-500">
    ...
    <button onClick={reset} className="px-8 py-3 bg-white/10 hover:bg-white/20 border border-white/10 text-white rounded-xl transition-all font-medium mt-8">
      Kembali ke Beranda
    </button>
  </div>
);
...
const AttendanceWizard: React.FC = () => {
  ...
  const handleTimeComplete = (timeData: { sessionName: string, status: 'Hadir'|'Terlambat' }) => {
    setData(prev => ({ ...prev, ...timeData }));
    setStep(2);
  };

  const handleLocationComplete = (locData: { lat: number, lng: number }) => {
    setData(prev => ({ ...prev, location: locData }));
    setStep(3);
  };

  const handleQRComplete = (qrData: { nim: string, name: string, deviceId: string }) => {
    setData(prev => ({ ...prev, ...qrData }));
    setStep(4);
  };

  const handleSelfieComplete = (photoBase64: string) => {
    const finalData = { ...data, photoBase64 } as Omit<Log, 'id' | 'timestamp'>;
    addLog(finalData);
    setStep(5);
  };
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col font-sans text-slate-100 overflow-hidden relative">
      ...
      <header className="w-full p-6 flex justify-between items-center relative z-10 border-b border-white/5 bg-slate-950/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-cyan-400 to-purple-600 rounded-xl flex items-center justify-center shadow-lg">
            <span className="font-bold text-white text-xl tracking-tighter">A.</span>
          </div>
        ...
        <div className="text-xs font-medium px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-slate-300">
          Portal Mahasiswa
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col relative z-10 w-full max-w-4xl mx-auto px-4 py-8">
        {step < 5 && (
          <div className="mb-12">
            <div className="flex justify-between relative">
              ...
              <div 
                className="absolute top-1/2 -translate-y-1/2 left-0 h-1 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full transition-all duration-500"
                ...
                return (
                  <div key={label} className="relative z-10 flex flex-col items-center gap-2">
                    <div className={cn(
                      ...
                      isActive ? "bg-slate-900 border-cyan-400 text-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.5)]" : 
                      isPassed ? "bg-cyan-500 border-cyan-500 text-white" : "bg-slate-900 border-white/20 text-white/30"
                    )}>
                      {isPassed ? <CheckCircle2 className="w-4 h-4" /> : idx + 1}
                    </div>
                    <span className={cn("text-xs font-medium absolute -bottom-6 w-max", isActive ? "text-cyan-400" : isPassed ? "text-slate-300" : "text-slate-600")}>
                      {label}
                    </span>
                  </div>
                );
        ...
        <div className="flex-1 flex items-center justify-center">
          {step === 1 && <TimeCheck onComplete={handleTimeComplete} />}
          {step === 2 && <LocationCheck onComplete={handleLocationComplete} />}
          {step === 3 && <QRScanner onComplete={handleQRComplete} />}
          {step === 4 && <SelfieCapture onComplete={handleSelfieComplete} />}
...
const AdminLogin: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulate Next.js Server Action / Auth logic
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="flex flex-col items-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-br from-cyan-400 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg mb-4">
             <span className="font-bold text-white text-3xl tracking-tighter">A.</span>
          </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="space-y-1">
            ...
            <input type="text" value={user} onChange={e=>setUser(e.target.value)} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-cyan-500 transition-colors" placeholder="admin" required />
          </div>
          <div className="space-y-1">
          ...
          <button type="submit" className="w-full py-3.5 mt-2 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-bold rounded-xl transition-all shadow-[0_0_15px_rgba(6,182,212,0.2)]">
            Masuk Dashboard
          </button>
        </form>
      </div>
...
const AdminDashboardHome: React.FC = () => {
  ...
  const sessionCounts = todayLogs.reduce((acc, log) => {
    acc[log.sessionName] = (acc[log.sessionName] || 0) + 1;
    return acc;
...
const AdminSettings: React.FC = () => {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    setIsAdding(false);
    setNewSess({ name: '', startTime: '', endTime: '', toleranceMinutes: 15 });
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white">Manajemen Sesi</h2>
          <p className="text-slate-400 text-sm">Atur jadwal absensi yang berlaku.</p>
        </div>
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-white/5 border border-cyan-500/30 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-5 gap-4 items-end animate-in slide-in-from-top-4">
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Nama Sesi</label>
            <input required type="text" value={newSess.name} onChange={e=>setNewSess({...newSess, name: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500" placeholder="e.g. Kuliah Malam" />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Jam Mulai</label>
            <input required type="time" value={newSess.startTime} onChange={e=>setNewSess({...newSess, startTime: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500" />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Jam Selesai</label>
            <input required type="time" value={newSess.endTime} onChange={e=>setNewSess({...newSess, endTime: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500" />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Toleransi (Menit)</label>
            <input required type="number" min="0" value={newSess.toleranceMinutes} onChange={e=>setNewSess({...newSess, toleranceMinutes: parseInt(e.target.value)})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500" />
          </div>
        ...
        {sessions.map(session => (
          <div key={session.id} className={cn("p-5 rounded-2xl border transition-colors", session.isActive ? "bg-white/5 border-white/10" : "bg-white/5 opacity-50 border-white/5")}>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => updateSession(session.id, { isActive: !session.isActive })}
                  className={cn("px-3 py-1 text-xs font-semibold rounded-full border", session.isActive ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-slate-500/20 text-slate-400 border-slate-500/30")}
                >
                  {session.isActive ? 'Aktif' : 'Nonaktif'}
                </button>
                <button onClick={() => deleteSession(session.id)} className="p-1 text-slate-400 hover:text-rose-400 transition-colors">
    ...
    <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Data histori absensi mahasiswa.</p>
        </div>
        <button 
          onClick={() => exportToCSV(filteredLogs)}
          className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white rounded-xl transition-all font-medium shadow-lg"
        >
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <input 
          ...
          className="flex-1 bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500"
        />
        <select 
          value={filterSession} 
          onChange={e=>setFilterSession(e.target.value)}
          className="bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-cyan-500 w-full md:w-48 appearance-none"
        >
                ...
                <tr key={log.id} className="hover:bg-white/5 transition-colors">
                  <td className="p-4">
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-white/10 bg-black relative group">
                      ...
                      {/* Hover Preview */}
                      <div className="absolute hidden group-hover:block w-48 h-64 z-50 left-12 top-0 border-2 border-white/20 rounded-xl overflow-hidden shadow-2xl bg-black">
                         <img src={log.photoBase64} alt="Selfie Large" className="w-full h-full object-cover" />
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                    <p className="font-semibold text-white">{log.name}</p>
                    <p className="text-xs text-slate-400 font-mono">{log.nim}</p>
                  </td>
                  <td className="p-4">
                    <p className="text-slate-200">{new Date(log.timestamp).toLocaleTimeString('id-ID')}</p>
                    <p className="text-xs text-slate-400">{new Date(log.timestamp).toLocaleDateString('id-ID')}</p>
                  </td>
                  <td className="p-4 text-slate-300">{log.sessionName}</td>
                  <td className="p-4">
                    <span className={cn(
                      "px-3 py-1 text-xs font-bold rounded-full",
                      log.status === 'Hadir' ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"
                    )}>
                      {log.status}
                    </span>
                  </td>
...
const AdminLayout: React.FC<{ children: React.ReactNode, activeRoute: string, setRoute: (r:string)=>void }> = ({ children, activeRoute, setRoute }) => {
  ...
  const navItems = [
    ...
    { id: 'admin-reports', icon: FileText, label: 'Laporan Data' },
    ...
    <div className="min-h-screen bg-slate-950 flex text-slate-200 font-sans">
      ...
      <aside className="w-72 bg-slate-900/50 border-r border-white/10 flex flex-col backdrop-blur-xl">
        <div className="p-6 border-b border-white/10 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-xl tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">AXAXYZ</h1>
            <p className="text-[10px] uppercase tracking-widest text-cyan-500 font-bold">Admin Portal</p>
          </div>
        </div>
          ...
          {navItems.map(item => (
            <button
              ...
              className={cn(
                ...
                activeRoute === item.id 
                  ? "bg-gradient-to-r from-cyan-500/20 to-purple-500/20 text-cyan-400 border border-cyan-500/30 shadow-[inset_0_0_20px_rgba(6,182,212,0.1)]" 
        ...
        <div className="p-4 border-t border-white/10">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-rose-400 hover:bg-rose-500/10 transition-colors text-sm font-medium">
...
export default function App() {
  // Simple state-based routing for the single-file environment
  // Valid routes: 'student', 'admin-login', 'admin-dashboard', 'admin-settings', 'admin-reports'
  const [route, setRoute] = useState<string>('student');

  useEffect(() => {
    // Check initial admin auth state if trying to access admin
    const isAdminAuthed = localStorage.getItem('axaxyz_admin_auth') === 'true';
    if (route.startsWith('admin-') && route !== 'admin-login' && !isAdminAuthed) {
      setRoute('admin-login');
    }
  }, [route]);
  ...
  return (
    <AppProvider>
      {/* Dev Navigation Toggle for Preview Environment (Remove in production) */}
      <div className="fixed bottom-4 right-4 z-50 flex gap-2 bg-slate-900/80 backdrop-blur-md p-2 rounded-xl border border-white/10 shadow-2xl">
        <button 
          ...
          className={cn("px-4 py-2 rounded-lg text-xs font-bold transition-colors", route === 'student' ? "bg-cyan-500 text-white" : "bg-white/10 text-slate-300 hover:bg-white/20")}
        >
          Mode Mahasiswa
        </button>
        <button 
          onClick={() => setRoute(localStorage.getItem('axaxyz_admin_auth') === 'true' ? 'admin-dashboard' : 'admin-login')}
          className={cn("px-4 py-2 rounded-lg text-xs font-bold transition-colors", route.startsWith('admin') ? "bg-purple-500 text-white" : "bg-white/10 text-slate-300 hover:bg-white/20")}
        >
          Mode Admin
        </button>
      </div>
      ...
      {['admin-dashboard', 'admin-settings', 'admin-reports'].includes(route) && (
